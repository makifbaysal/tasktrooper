...
| `ReleasesCard` | Organism, below the environment/runtime pieces for EVERY component (mobile included): the selected component's last 20 releases (`listReleases`), newest first — version (monospace), status chip, task count, created relative time, `deploy.run_url` when present. For a `batch` component, a draft row is pinned first — "Next release — N merged tasks" with a **Cut release** button (hidden at N = 0, opens `CutReleaseDialog`). Polls every 15s while any listed release is `RELEASE_ACTIVE_STATUSES` (deploying/verifying/rolling_back/awaiting_verdict). Row click opens `ReleaseDrawer`. |
...
"confirmed \<relative time\>" instead.
