...
| `release-engineer` | `generalPurpose` | medium | Merge, ship, verify and roll back — owns `done`/`released` (role `release`, migration 159) |

Migration 159 moved `done`/`released` from QA to the release engineer: QA's
`agent_column_subscriptions` for both were deleted and `workflow_stage_participants`
repointed to role `release`, so QA's own work now ends at its verdict in
`in_qa`/`pm_uat`/`human_uat`. See "Releases" below.
...
1. **Manual verification** — scenarios come from the task definition + acceptance criteria, without reading code (`scenario-plan-first`); the environment is chosen (`test-environment-selection`: the task's own Vercel preview from `get_task_preview` when it is ready and built from the PR head; otherwise a local boot in the task workspace; if the repo maps a stage, `get_deploy_target` gives the stage `base_url`). Backend: boot API + worker, real requests, side-effect checks (`backend-manual-testing`, `worker-job-testing`). Frontend: drive the flow in headless Chromium, desktop/mobile screenshots, visual checklist (`frontend-manual-testing`, `ui-visual-evidence` rule).
...
2. **A task landing in `done` wakes the release engineer** — to merge it, via `merge_task_pull_request` (squash + branch delete). To keep the old bug out, four conditions must all hold (`board/dispatcher.go` `doneMergeWake`): column is `done`, the task type ships code (not analiz), the event is `task.moved`/`task.created` (not comment/assign/update — `UpdateTask` already emits `task.moved` only on a real column change), and the task has a recorded, **not yet merged** PR (`pr_url` set, `merge_commit_sha` empty). QA used to hold this wake; migration 159 moved the `done`/`released` subscriptions to role `release` instead (see "Releases" below).

The woken agent resolves from the **column subscription** (the release engineer), never the assignee: `resolveAgents` returns the implementer for `done`, which is exactly the behaviour that produced the slide into `released`. One-shot is guaranteed by `merge_commit_sha` (migration 104) — the merge writes it, and every later event finds "nothing to do". Also: an agent is not dispatched on its own event, `alreadyWorkingOn` checks **running** runs on this path too (no two concurrent merges), and reconciler sweeps already skip `done`.

Three run-side adaptations: `producesADiff` (`board/review.go`) excludes `done` — a post-run commit after the merge would recreate the deleted branch on origin seconds later; `done`'s `strip_writers` stage behaviour (`domain.RestrictToolsForStage`, not the old `RestrictToolsForVerdictColumn`/`domain.verdictColumns` names) strips the file writers and `commit_task_changes` but its `allow` param keeps `merge_task_pull_request` and the `release_control` tools (`deploy_release`/`finish_release`/`rollback_release`); and `columnInstruction`'s `done` case drops the default's "move to the next column" sentence and instead walks the release-engineer flow end to end (merge → act on `release.mode` → what to do when woken).
...
### `done` / `released` = the columns where the release is watched (`deployWatchWake`, migrations 105 and 159–161)
...
The release engineer completes the same sequence in `done` — **merge → deploy → watch → verify → finish or roll back** — and also holds the `released` subscription, because a release's own health-window incident can wake a card back there. Migration 159 moved both subscriptions from QA to role `release`.

**The wake is narrower than a state check.** `deployWatchWake` does not look at the task's *state*, only at a payload key that only the sweeper (and, for the legacy per-task path, the rollback dispatcher) writes: `domain.EventPayloadResumedResource` equal to `"deploy_watch"` **or** `"release_watch"` — `release.Service`'s sweeper writes the latter (`board.NewReleaseWaker`, wired as the release package's `Waker`). A state-based condition — "in done, merged, deploy unverified" — would recur on every comment and every update to the card and wake the release engineer forever; the payload key can only be produced by the one caller that writes it. The column check still runs (`done`/`released`), and so does the type check (analiz ships no code).

**A pending release parks, it does not block.** `watch_release` returns `domain.ResourceBlock{Resource: "release_watch"}` instead of a result whenever the release is `Watched()` (deploying/verifying/rolling_back); the loop ends its turn, the runner parks the card, and `release.Service`'s sweeper (`SweepOnce`, 30s) brings it back when the deploy settles or the soak window ends. Same mechanism as the device park, with one structural difference: a device is a QUEUE (one phone; whoever frees it releases the next, oldest first), whereas two parked releases are two separate things and either may finish first — so the sweeper claims only the ready ones (`TakeBlockedResourceTask`). Waiting costs one API call per park per pass and zero tokens; a retry loop on the agent side would spend one model turn per poll and hold a concurrency slot for the whole deploy.

**Run side:** `columnInstruction`'s `done` case walks the release-engineer flow (merge, act on `release.mode`, what to do when woken with `pending`/`awaiting_verdict`/`failed`), and `released` has its own case restricted to a health-incident wake (the default's "move to the next column" must never reach either — there is no column after `released`, and `done` is reached only through the merge wake or a release hand-back). `domain.RestrictToolsForStage` (not the old `RestrictToolsForVerdictColumn`) strips writers on every stage carrying `strip_writers`; its `allow` param decides which of `merge_task_pull_request`/`release_control` survive per stage — `done` keeps both, `released` keeps only `release_control` (`deploy_release`/`finish_release`/`rollback_release`, `domain.ReleaseControlTools`).

The tools themselves, their refusal matrices and the release state machine: `.ai/tool-reference.md` → `get_release` / `deploy_release` / `watch_release` / `finish_release` / `rollback_release`, and `.ai/architecture.md` → "Releases (migrations 159–161)".
...
| Situation | Action |
|---|---|
| a failed job exists (`github_actions`) | `get_deploy_logs`, then `rollback_release` (reason `deploy_failed`) if the bad code is live or sitting on the default branch |
| a batch `local` run failed | read `get_release`'s `local_run.tail` (and its log path) instead of `get_deploy_logs` |
| a batch `store` build failed | `get_release`'s `store_builds` names the failing platform and why |
| nothing actually shipped, nothing to undo | report what failed on the card and stop — a `failed` release is never silently left for the next sweep to retry |
| merge conflict (`dirty`/`behind`) | the task goes to `need_revision` — rebasing is the developer's job |

A merged-but-undeployed task is never left silently in `done`: `rollback_release` or a comment naming what failed, always one or the other.
...
`repositories.require_review_chain`, `require_release_deploy` and `require_pipeline_for_review` are gone — dropped by migration 158 along with the `PUT /v1/repositories/:id/lifecycle-gates` endpoint that armed them. What each used to gate:

- **The review chain** blocks a move into `done` — and into `released` when that would skip `done`, but not the ordinary `done → released` promotion, since `done` already asserted the chain — unless the task has actually visited every stage `Workflow.ReviewChain()` lists for its type (built from each stage's `review_chain_stage` behaviour): `code_review` → `in_qa` → `pm_uat` for `task`/`bug`, `analiz_review` alone for `analiz`. Evidence is whether the task's span history (`task_column_spans`, migration 057) ever contains that column, **not** the current column and **not** a verdict field — so rework through `need_revision` and back is never punished, it just re-earns the stage. Where a verdict *was* recorded (only while `require_human_review` is on), a stage whose most recent visit ended in a recorded `reject` blocks too, even if the column was visited. A stage whose column is missing from this board's `board_columns` is skipped rather than counted as failed — a customized board cannot route a task through a column it does not have. This gate is now **always on**: `internal/domain/lifecycle_gate.go`, `internal/application/repository/lifecyclegate.go` (`reviewChainGate`/`CheckReviewChain`) no longer check a repository flag before enforcing it.
- **The release-deploy gate is gone, replaced by the release state machine.** A move into `released` is never a bare column-transition check any more; it happens only through `finish_release`'s verdict (or a `none`-mode merge that is the release itself) — see [architecture.md](architecture.md) → "Releases (migrations 159–161)".
- **The pipeline gate** (code_review waits for the build/test pipeline before the reviewing architect is dispatched) is now unconditional wherever a stage carries `wait_for_ci` — there is no more per-repository off switch. It is still bounded by `board.pipeline_gate_timeout`, so it cannot deadlock a repository whose CI cannot answer; see [architecture.md](architecture.md) → "The code-review gate has a bounded life".
- **The review chain gate still fails closed** on evidence it cannot read (span store unavailable, or a lookup error) — the same direction every other board gate in this file fails, since a check that passes when its input is missing is not a check. The block error names the missing/rejected stage and the remedy move, the same way `ErrMigrationNotStaged` does for the migration gate.
- **Repo-level coverage/mutation columns** (`require_overall_coverage`, `coverage_threshold`, `mutation_enabled`, `mutation_threshold`) stay on `repositories`, but they are no longer user-settable: they are a projection of the single component's gates (`application/projectmodel/projection.go` → `projectQualityGates`), same pattern as `kind`/`sub_repo_kinds`/`sub_projects`. A monorepo always projects the all-off default onto these columns, since each sub-project carries its own gates via `sub_projects`.
