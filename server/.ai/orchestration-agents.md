...
- **The release-deploy gate is gone.** A move into `released` is never gated on a recorded production deploy any more; `trigger_release`/`TriggerRelease` fires from `done` unconditionally, whether or not the repository has a deploy package, a prod workflow mapped, or any deploy history at all.
