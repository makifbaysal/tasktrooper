...
// Probe reports whether this host can actually run an OpenCode session.
func Probe(ctx context.Context, binary string) (ProbeResult, error) {
	resolved, err := ResolveBinary(binary)
	if err != nil {
		return ProbeResult{}, fmt.Errorf("%s. Install the OpenCode CLI on this machine, or set OPENCODE_BIN to its path: %w",
...
func probeAuth(ctx context.Context, bin string) error {
	...
	cmd := exec.CommandContext(ctx, bin, "run", probePrompt, "--format", "json")
	cmd.Env = childEnv(ctx)
	cmd.Dir = dir
	var buf bytes.Buffer
	cmd.Stdout = &buf
