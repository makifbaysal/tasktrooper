...
- **`unconfirmed`** — the component's delivery profile was never confirmed; the system already posted the one comment explaining that. Nothing to do.
- **`batch`** — mobile and other batched components queue merges for a human to cut later. The task stays in `done`. Nothing to do.
...
1. `get_release` for the checks already gathered (health samples, smoke results, new error groups, notes, early stop).
2. `query_runtime_logs` since `deployed_at` — read what the running service actually logged after this deploy.
3. `list_runtime_errors` for grouped errors that are new since the deploy.
4. Where the task's acceptance criteria imply something worth checking read-only, use `browser_navigate`/`fetch_url` against the production URL — GET/HEAD only, never a state-changing request.

A release finished without having read logs and errors in THIS run is not verified — it is a guess that happened to look green.
